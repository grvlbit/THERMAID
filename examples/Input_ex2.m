...
udata.Nf      = [101 101];  	                % number of cells in x and y direction
udata.dx      = udata.len./udata.Nf;                      % cell length [m]

%% SIMULATION PARAMETER FOR TRANSPORT --------------------------------------------------------------%
udata.timeSim  = 40*365.25*86400;                         % total simulation time [s]
udata.dt       = 365.25*86400;                           % time step length [s]
udata.tol    = 1.e-4;                       % saturation tolerance on pressure-concentration-heat loop [-]
...
%% INITIAL CONDITIONS--------------------------------------------------------------------------------%   
udata.T0   = 180*ones(udata.Nf(1),udata.Nf(2));              % Initial matrix temperature [°C]
udata.T0f  = 180*ones(udata.Nf_f,1);                   % Initial fracture temperature [°C]
udata.tmax = 180;                              % maximum temperature [°C] for plotting
...
a = 1e-3;
k0 = 1e-16;
udata.K       = ones(udata.Nf(1),udata.Nf(2))*k0; 	    % permeability field [m2]
udata.K_f     = ones(udata.Nf_f,1)*k0*k_ratio; 	        % fracture permeability field [m2]
...
udata.compressibility_l = 5e-1000;                        % Bulk Modulus of the fluid [Pa]
udata.compressibility_s = 5e-1000;                        % Bulk Modulus of the rock  [Pa]
...
udata.K_enh = 1;				     % Permeability enhancement factor [-]
